...
export default function Header({ onAuthClick, currentLocation, onLocationChange, cartCount, onCartOpen, user, onNavigate }) {
    ...
    const filteredProvinces = Object.keys(LOCATION_DATA).filter(prov => prov.includes(provinceSearch));
    const cities = tempProvince ? LOCATION_DATA[tempProvince] : [];
    const filteredCities = cities.filter(city => city.includes(citySearch));
    ...
    return (
        <>
            <header className="sticky top-0 z-40 bg-white border-b border-gray-100 text-right" dir="rtl">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-4 shrink-0">
                        ...
                        >
                            ...
                            <span className="font-black text-gray-800 text-xs truncate max-w-[120px]">
                    ...
                    <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                        ...
                        {/* دکمه پروفایل کاربری با عرض کاملاً هوشمند */}
                        {user ? (
                            <button
                                onClick={() => onNavigate('profile')}
                                className="flex items-center gap-2 bg-brand-pink/5 hover:bg-brand-pink/10 text-brand-pink px-4 py-2 rounded-xl font-black text-xs border border-brand-pink/10 transition-all cursor-pointer w-auto shrink-0 whitespace-nowrap"
                            >
            ...
            {isLocationModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
                    ...
                    <div ref={modalRef} className="w-full max-w-2xl rounded-[32px] p-6 relative z-10 text-right bg-white border border-gray-100 shadow-2xl flex flex-col max-h-[85vh]" dir="rtl">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 overflow-hidden flex-1 min-h-[320px]">
                            ...
                            <div className="flex flex-col bg-gray-50 border border-gray-100 rounded-2xl p-4 overflow-hidden">
                                ...
                                <div className="overflow-y-auto flex-1 space-y-1 pr-1 scrollbar-none">
                                    {filteredProvinces.map(prov => (
                                        <button
                                            key={prov}
                                            onClick={() => { setTempProvince(prov); setTempCity(''); }}
                                            className={`w-full text-right px-3 py-2.5 rounded-xl font-bold text-xs flex items-center justify-between transition-all cursor-pointer ${tempProvince === prov ? 'bg-brand-pink/10 text-brand-pink font-black' : 'text-gray-700 hover:bg-gray-100'}`}
                                        >
                            ...
                            {/* انتخاب شهر */}
                            <div className="flex flex-col bg-gray-50 border border-gray-100 rounded-2xl p-4 overflow-hidden">
                                <label className="text-xs font-black text-gray-500 mb-2 block">۲. شهرهای {tempProvince || 'استان انتخاب شده'}</label>
                                <div className="relative flex items-center mb-3">
                                    <Search size={16} className="absolute right-3 text-gray-400" />
                                    <input
                                        type="text"
                                ...
                                <div className="overflow-y-auto flex-1 space-y-1 pr-1 scrollbar-none">
                                    {tempProvince && filteredCities.map(city => (
                                        <button
                                            ...
                                            className={`w-full text-right px-3 py-2.5 rounded-xl font-bold text-xs flex items-center justify-between transition-all cursor-pointer ${tempCity === city ? 'bg-purple-500/10 text-purple-600 font-black' : 'text-gray-700 hover:bg-gray-100'}`}
                                        >
                                            <span>{city}</span>
                        ...
                        <div className="border-t border-gray-200 pt-4 mt-4 flex items-center justify-between">
                            <div className="text-xs text-gray-500 font-bold">
                                {tempProvince && tempCity ? <span>محدوده: <strong className="text-gray-800">{tempProvince}، {tempCity}</strong></span> : <span className="text-red-500">لطفاً شهر خود را انتخاب کنید.</span>}
                            </div>
                            <button disabled={!tempProvince || !tempCity} onClick={handleConfirmLocation} className="bg-gray-900 hover:bg-gray-800 text-white font-black text-xs px-6 py-3 rounded-xl shadow-md transition-all cursor-pointer disabled:opacity-40">
                                تایید و اعمال آدرس
                            </button>
                        </div>
                    </div>
